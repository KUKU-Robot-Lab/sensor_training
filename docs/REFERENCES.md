...
| 논문 | 링크 | robot_skin 이 가져오는 것 | 코드 위치 |
|---|---|---|---|
| **Masked Autoencoders Are Scalable Vision Learners** (MAE) — He et al., CVPR 2022 | [arXiv:2111.06377](https://arxiv.org/abs/2111.06377) | 입력의 높은 비율을 가리고 재구성하는 자기지도 사전학습이다. robot_skin 은 이를 taxel 에 적용했다: taxel 을 가리고 `[MASK]` 값 벡터로 residual_z 와 접촉 level 을 재구성한다. 고정 2-D sin-cos 위치 임베딩도 여기서 가져왔다. | `representation/pretrain.py` (`MaskedTaxelPretrainer`), `stages/pretrain.py`, `representation/tokenizer.py`, `vision/encoders.py` |
