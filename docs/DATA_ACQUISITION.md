...
| 데이터셋 | 내용 | 쓰임 |
|---|---|---|
| **D1 `motion`** (`protocols/d1_motion.yaml`) | 물체 없이 손만 움직임: 정지 baseline, IMU 보정, 손가락별 굴곡, 쥐기/펴기(느림·빠름), 손목 회전, 자유 동작, **자기접촉 세트**, 3-탭 싱크 | IMU → 손 자세 모델, 움직임에 의한 촉각 baseline 예측기(무접촉 구간), 접촉 검출기 보정(자기접촉 = 공짜 접촉 라벨) |
...
한 세션 ≈ 블록 4.4 분 (명목 휴지 1 s 포함 4.9 분) + 운영자 휴지 (실제 8–10 분). 각 블록 전 대본을 읽고 **Enter 로 시작**하면
...
| 15–18 | `pinch_{index,middle,ring,pinky}` | 4 × 10 s | 자기접촉 → `self_touch` | 엄지 끝과 각 손가락 끝을 살짝→꾹 눌렀다 뗌 (2 s × 5회) |
| 19 | `fist` | 12.5 s | 자기접촉 | 주먹 꽉 쥐기(손끝이 손바닥을 누름)/완전히 펴기 (2.5 s × 5회) |
| 20 | `finger_crossing` | 10 s | 자기접촉 | 검지·중지 꼬기/풀기 (2.5 s × 4회) |
| 21–25 | `palm_touch_{thumb,index,middle,ring,pinky}` | 5 × 8 s | 자기접촉 | 각 손가락 끝을 같은 손 손바닥에 굽혀 눌렀다 폄 (2 s × 4회) |
| 26 | `sync_end` | ≈4.3 s | 탭 | 3-탭 싱크 반복 (드리프트 측정) |
| 27 | `baseline_end` | 5 s | 없음 | 휴식 자세 정지 (baseline 드리프트 확인) |
